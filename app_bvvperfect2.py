...
if st.session_state.df is None:
    ...
    if page == "📋 Data View":
        ...
        if not KEYBOARD_AUTOMATION_AVAILABLE:
            ...
            # Add row index column for selection
            display_df_with_index = display_df.reset_index(drop=True)
            display_df_with_index.index.name = "Row"
            
            # Display dataframe (like Data View)
            st.dataframe(display_df_with_index, use_container_width=True, height=400)
            st.caption(f"Showing {len(display_df)} of {len(original_df)} rows. Click on a row number below to select it.")
            
            # Row selection interface
            if len(display_df) > 0:
                # Create row options with key information
                row_options = []
                for idx, row in display_df_with_index.iterrows():
                    # Build a label with key info
                    label_parts = []
                    if number_col and number_col in row.index:
                        label_parts.append(f"Card: {str(row[number_col])[:20]}")
                    # Add index
                    label_parts.append(f"(Row {idx})")
                    row_options.append(f"{' | '.join(label_parts)}")
                
                # Select row
                selected_option = st.selectbox(
                    "Select a row to test:",
                    options=range(len(display_df_with_index)),
                    format_func=lambda x: row_options[x] if x < len(row_options) else f"Row {x}",
                    key="test_card_row_selector",
                    index=st.session_state.selected_test_row_index if st.session_state.selected_test_row_index is not None and st.session_state.selected_test_row_index < len(display_df_with_index) else 0
                )
                ...
                selected_row = display_df_with_index.iloc[selected_option]
